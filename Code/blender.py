...
def process_line(line):
	'''
	Read the Blender3D object file, line by line.
	'''
	label = None
	fields = line.split()
	if len(fields) == 0:
	...
	# check first character of each line and label appropriately, handling any in incorrect format.
	if fields[0] == '#':
		label = 'comment'
		...
		if len(fields) != 4 and len(fields) != 5:
			...
			return None
		'''
		face lines can have multiple formats, e.g. f 586/1 1860/2 1781/3 or f vi/ti/ni, where where vi is the vertex
		index, ti is the texture index and ni is the normal index
		'''
...
def load_material_library(file_name):
	'''
	Read the Blender3D material file, line by line.
	'''
...
def create_meshes_from_blender(vlist, flist, mlist, tlist, library, mesh_list, lnlist):
	...
	# put all texture vectors in one array.
